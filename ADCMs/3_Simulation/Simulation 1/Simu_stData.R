...
Simu_stData <- function(para, site = NULL, W_ts = NULL){
  ...
  if(is.null(site)){
    ...
    library(sp)
    site <- spCoords.transform(loc, method  = 1)
    # plot(site[, 2:3])
  ...
  X_ts <- array(0, dim = c(9, para$Nt, para$n),
  ...
  re <- list(Site = site, Y_ts = Y_ts - mean(g1(X_ts[4,,])) - mean(g0(X_ts[5,,])) , 
             X_ts = X_ts, W_ts = W_ts,
             g0 = g0(as.vector(X_ts[4,,])) - mean(g0(X_ts[4,,])),
             g1 = g1(as.vector(X_ts[5,,])) - mean(g1(X_ts[5,,])),                       
             Site = site,
             siteid = "ID")
  return(re)
}
